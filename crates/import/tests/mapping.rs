...
use import::testkit::{buy_row, sale_row, vest_row, founding_residency};
...
fn buy_row_reconstructs_buy_with_tranche_lot_id_and_basis_from_dps_plus_fees() {
    ...
    wb.actions.push(buy_row(
        "Stock Actions", 2, "B1", "GOOG", 100, "50.00", "7.00", date(2020, 1, 15), PLATFORM,
    ));

    let recon = reconstruct(&wb).expect("clean buy reconstructs");
    let buy = recon.ledger.iter().find(|e| matches!(e.event.kind, LedgerEventKind::Buy { .. }))
        .expect("a Buy event");
    match &buy.event.kind {
        LedgerEventKind::Buy { lot_id, symbol, qty, unit_price_cents, fees_cents, .. } => {
...
fn vest_row_reconstructs_vest_with_fmv_recovered_from_dps_not_zero_total_cost() {
    ...
    let vest = recon.ledger.iter().find(|e| matches!(e.event.kind, LedgerEventKind::Vest { .. }))
        .expect("a Vest event");
    match &vest.event.kind {
        LedgerEventKind::Vest { lot_id, symbol, qty, fmv_per_share_cents, .. } => {
            assert_eq!(lot_id, "AMZN-V1");
            assert_eq!(symbol, "AMZN");
            assert_eq!(*qty, MicroShares(100_000_000));
            assert_eq!(*fmv_per_share_cents, Cents(200_000), "$2000 FMV recovered, not $0");
        }
...
fn sell_to_cover_child_becomes_ordinary_sell_at_vest_date_price() {
    ...
    wb.actions.push(vest_row(
        "Stock Actions", 2, "RSU1", "MSFT", 100, "300.00", date(2022, 5, 1), PLATFORM,
    ));
    // The sell-to-cover child: 40 shares @ the same $300 vest-date price.
    wb.sales.push(sale_row(
        "Stock Sales", 2, "RSU1-a", "RSU1", "MSFT", 40, "300.00", "0",
        date(2022, 5, 1), PLATFORM,
    ));

    let recon = reconstruct(&wb).expect("vest + sell-to-cover reconstructs");
    let sell = recon.ledger.iter().find(|e| matches!(e.event.kind, LedgerEventKind::Sell { .. }))
        .expect("a Sell event for the sell-to-cover");
    match &sell.event.kind {
        LedgerEventKind::Sell { symbol, qty, unit_price_cents, lot_refs, .. } => {
...
fn sale_row_reconstructs_specific_id_sell_against_referenced_tranche() {
    ...
    let sell = recon.ledger.iter().find(|e| matches!(e.event.kind, LedgerEventKind::Sell { .. }))
        .expect("a Sell event");
    match &sell.event.kind {
        LedgerEventKind::Sell { symbol, qty, unit_price_cents, fees_cents, lot_refs, .. } => {
            assert_eq!(symbol, "GOOG");
...
fn positions_tab_is_reconcile_target_only_never_a_reconstruction_source() {
    ...
    assert!(a.ledger.iter().any(|e| matches!(
        &e.event.kind,
