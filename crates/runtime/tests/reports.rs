...
fn series_point(key_days: i32, mv_cents: i64) -> SeriesPoint {
    ...
    marks.insert("AMZN".to_string(), PricedMark { price_cents: Cents(170_00), quote_epoch: Date(key_days) });
...
fn a_transient_write_failure_is_retried_under_the_loop_then_succeeds() {
    ...
    let policy = BackoffPolicy { max_attempts: 5, base_delay: Duration::from_millis(1), max_delay: Duration::from_millis(8) };
    let slept: RefCell<Vec<Duration>> = RefCell::new(Vec::new());

    let point = series_point(19_180, 510_00);
    let outcome = capture_with_retry(&mut history, &lock, &point, &policy, |d| slept.borrow_mut().push(d));

    assert_eq!(outcome, CaptureOutcome::Captured, "the point lands on a retry");
    assert!(outcome.is_captured());
    // Exactly three upsert attempts (fail, fail, succeed) and two backoff sleeps.
    assert_eq!(*history.upsert_calls.borrow(), 3, "the loop retried under failure");
...
fn continued_failure_surfaces_the_point_as_uncaptured_not_dropped() {
    ...
    let lock = NoopLock::new();
    let policy = BackoffPolicy { max_attempts: 3, base_delay: Duration::from_millis(1), max_delay: Duration::from_millis(1) };
    ...
    match outcome {
        CaptureOutcome::Uncaptured { key, attempts } => {
            assert_eq!(key, point.key, "the uncaptured flag carries the trading-day key");
    ...
    assert_eq!(history.row_count(), 0, "no point landed (it is flagged uncaptured)");
}
...
fn a_held_write_lock_is_retried_then_flagged_uncaptured() {
    ...
    let mut history = InMemoryHistory::new();
    let policy = BackoffPolicy { max_attempts: 2, base_delay: Duration::from_millis(1), max_delay: Duration::from_millis(1) };
    let outcome = capture_with_retry(&mut history, &HeldLock, &series_point(19_182, 1), &policy, |_| {});
    assert!(outcome.is_uncaptured(), "a persistently-held lock flags the point uncaptured");
    assert_eq!(history.row_count(), 0, "nothing appended while the lock is held");
}
...
fn a_priced_cycle_triggers_the_history_capture_under_the_lock() {
    ...
    assert_eq!(captured, Some(CaptureOutcome::Captured), "a priced cycle is captured");
    let series = read_history(&history).expect("read back");
    assert_eq!(series.len(), 1, "the priced cycle produced one durable point");
...
fn a_degraded_only_cycle_triggers_no_capture() {
    ...
    let out = run_cycle(&logs, &MarksCache::new(), &ctx, AS_OF, &client, settle).expect("cycle");
    assert_eq!(out.trading_day_key, None, "a degraded-only cycle keys nothing");
    ...
    assert_eq!(history.row_count(), 0, "nothing captured for a degraded-only cycle");
}
...
fn a_re_triggered_capture_for_the_same_trading_day_reconciles_last_wins() {
    ...
    let first = capture_with_retry(&mut history, &lock, &series_point(19_180, 500_00), &policy, |_| {});
    let second = capture_with_retry(&mut history, &lock, &series_point(19_180, 999_00), &policy, |_| {});
    assert!(first.is_captured() && second.is_captured());

    let series = read_history(&history).expect("read back");
    assert_eq!(series.len(), 1, "last-wins: the same trading day did not duplicate");
    assert_eq!(series[0].total_market_value_cents, Cents(999_00), "the later capture overwrote");
}
...
fn load_run_and_capture_drives_the_cycle_then_the_capture() {
    ...
    assert!(captured.outcome.snapshot.positions.contains_key("AMZN"), "the cycle replayed");
    assert_eq!(captured.capture, Some(CaptureOutcome::Captured), "the priced cycle was captured");
    assert_eq!(read_history(&history).expect("read back").len(), 1, "one durable point landed");
}
