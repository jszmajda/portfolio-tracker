...
fn sell_composes_a_sell_event_driving_the_lot_picker_allocation() {
    ...
    let mut picker = entry::LotPicker::build(
        &snap,
        &"AMZN".to_string(),
        "Robinhood",
        ms(40),
...
fn reversal_lists_targets_greying_reversed_and_prior_reversals_and_shows_blocks_inline() {
    ...
    assert!(
        buy_target.block_reason.is_some(),
        "reversing the Buy is blocked (the Sell consumed it)"
    );
    assert!(
        !buy_target.selectable(),
        "a blocked target is not selectable"
    );
    ...
    log2.push(ledger_core::LedgerEvent {
        ...
        kind: LedgerEventKind::Reversal {
            target_event_id: "e2".to_string(),
        },
    });
    let targets2 = entry::reversible_targets(&log2);
    assert!(
        targets2.iter().find(|c| c.event_id == "e2").unwrap().greyed,
        "the reversed Sell is greyed"
    );
    assert!(
        targets2.iter().find(|c| c.event_id == "e3").unwrap().greyed,
        "the prior Reversal is greyed"
    );
}
...
fn new_symbol_buy_warns_not_blocks_and_requires_confirm() {
    ...
    assert!(
        entry::symbol_is_known(&"AMZN".to_string(), &snap, &aliases),
        "AMZN is a known position"
    );
    assert!(
    ...
    assert!(
        warning.contains("[c]onfirm new position"),
        "requires a confirm step (does not block)"
    );
    ...
    assert!(
        entry::symbol_is_known(&"GOOG".to_string(), &snap, &aliased),
        "an aliased symbol is known"
    );
}
...
fn unconfirmed_new_symbol_buy_does_not_reach_confirmed_until_confirmed() {
    ...
    match entry::submit_buy(&mut rt, &unconfirmed, &aliases) {
        Phase::ConfirmNewSymbol(warn) => assert!(
            warn.contains("new symbol 'AMZM'"),
            "the warn surfaces: {warn}"
        ),
    ...
    assert_eq!(
        rt.ledger_log.len(),
        1,
        "the unconfirmed new-symbol Buy wrote nothing"
    );

    // The owner [c]onfirms the new position; the Buy now submits durably.
    unconfirmed.confirmed_new_symbol = true;
    assert_eq!(
        entry::submit_buy(&mut rt, &unconfirmed, &aliases),
        Phase::Confirmed,
        "a confirmed new symbol submits"
    );
    ...
    let known = BuyForm {
        symbol: "AMZN".to_string(),
        confirmed_new_symbol: false,
        lot_id: "L-amzn2".to_string(),
        ..unconfirmed.clone()
    };
    assert_eq!(
        entry::submit_buy(&mut rt, &known, &aliases),
        Phase::Confirmed,
        "a known symbol submits without confirm"
    );
}
...
fn manual_sell_with_no_residency_default_blocks_submit_until_explicit_state() {
    ...
    assert_eq!(
        rt.submit_count, 0,
        "the residency block never reaches the write path"
    );
...
fn manual_sell_with_a_resolved_residency_default_submits_without_blocking() {
    ...
    assert!(
        entry::sell_residency_guard(&form).is_none(),
        "a resolved default does not block"
    );
