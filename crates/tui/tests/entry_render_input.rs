...
fn entry_form_renders_rounded_titled_panel_with_caret_caps_labels_and_footer() {
    ...
    assert!(s.contains('\u{256D}'), "the rounded panel top-left corner ╭ is drawn");
    assert!(s.contains("Buy"), "the panel is titled by the flow");

    // Tracked-caps labels beside the inputs.
    assert!(s.contains("SYMBOL"), "the symbol label renders tracked-caps");
    assert!(s.contains("UNIT PRICE"), "the unit-price label renders tracked-caps");

    // The gilt focus caret sits on the focused (first) field.
    assert!(s.contains('\u{258E}'), "the ▎ focus caret renders on the focused field");

    // The footer hint line shows the active bracketed accelerators.
    assert!(s.contains("[enter] submit"), "the footer hints the submit accelerator");
    assert!(s.contains("[esc] cancel"), "the footer hints the cancel accelerator");
}
...
fn inline_validation_error_renders_in_the_one_slot_beneath_the_offending_field() {
    ...
    let rt = empty_runtime();
    let s = render_string(&model, &rt, 90, 24);
    assert!(s.contains("NonPositiveQty"), "the verified kernel error's words render inline: {s}");
    assert!(s.contains('\u{2717}'), "the ✗ error glyph travels with an error-role rejection");
}
...
fn submit_disagreement_reuses_the_same_inline_slot_as_inline_validation() {
    ...
    form.set_error(1, &InlineError::Ledger(ledger_core::LedgerError::NonPositiveQty));
    let first_field = form.error.as_ref().unwrap().field;
    // Then a submit-time disagreement on the same field overwrites the one slot.
    form.set_error(1, &InlineError::Ledger(ledger_core::LedgerError::InsufficientShares));
    assert_eq!(form.error.as_ref().unwrap().field, first_field, "the same single slot");
    assert!(
        form.error.as_ref().unwrap().message.contains("InsufficientShares"),
...
fn footer_accelerators_track_the_phase_retry_confirm_and_new_symbol() {
    ...
    assert!(form.footer_hint().contains("[r] retry"), "a returned-control submit offers [r]etry");

    form.set_phase(Phase::ConfirmNewSymbol("new symbol".to_string()));
    assert!(form.footer_hint().contains("[c] confirm"), "the new-symbol guard offers [c]onfirm");

    form.set_phase(Phase::Confirming);
    assert!(form.footer_hint().contains("[enter] confirm"), "a gated flow offers [enter] confirm");

    // A Sell offers [F] FIFO in the editing phase.
    let sell = FormModel::new(FlowKind::Sell, vec![Field::new("Qty", "0")]);
    assert!(sell.footer_hint().contains("[F] FIFO"), "the Sell/lot-picker offers FIFO-fill");
}
...
fn every_flow_kind_renders_a_titled_panel_chrome() {
    ...
    for kind in kinds {
        ...
        let mut model = Model::new();
        model.open_entry(EntryContext::form(FormModel::new(kind, vec![Field::new("Field", "x")])));
        let s = render_string(&model, &rt, 90, 16);
        assert!(s.contains(&title), "{kind:?} renders its titled panel: {title}");
        assert!(s.contains('\u{256D}'), "{kind:?} renders the rounded panel");
        assert!(s.contains("[esc] cancel"), "{kind:?} renders the footer hint");
    }
}
...
fn the_flows_with_field_builders_render_their_per_flow_labels_through_the_one_path() {
    ...
    assert_labels_render(&rt, FormModel::for_buy(&buy_form), &["SYMBOL", "QTY", "UNIT PRICE", "FEES"]);

    // Vest: FMV/share, no Fees.
    let vest_form = VestForm::with_defaults(&rt, "V1".to_string(), "AMZN", &platforms, &aliases);
    assert_labels_render(&rt, FormModel::for_vest(&vest_form), &["SYMBOL", "QTY", "FMV/SHARE", "PLATFORM"]);

    // Split: Symbol · Ratio · Date.
    let split_form = SplitForm { symbol: "AMZN".to_string(), ratio_num: 2, ratio_den: 1, date: Date(20_000) };
    assert_labels_render(&rt, FormModel::for_split(&split_form), &["SYMBOL", "RATIO", "DATE"]);

    // Sell: carries the accrues-to-state label.
    let sell_form = SellForm::with_defaults(&rt, "S1".to_string(), "AMZN", &residency, &platforms, &aliases);
    assert_labels_render(&rt, FormModel::for_sell(&sell_form), &["SYMBOL", "QTY", "ACCRUES TO"]);

    // Residency (a config flow with a real builder): Effective date · State.
    let res_form = tui::entry::ResidencyForm { effective_date: Date(20_000), state: "VA".to_string() };
    assert_labels_render(&rt, FormModel::for_residency(&res_form), &["EFFECTIVE DATE", "STATE"]);
}
...
fn assert_labels_render(rt: &FakeRuntime, form: FormModel, expected_labels: &[&str]) {
    ...
    for label in expected_labels {
        assert!(s.contains(label), "{kind:?} renders the per-flow label {label}: {s}");
    }
}
...
fn open_sell_picker_model(sale_qty: MicroShares) -> Model {
    ...
    let picker = LotPicker::build(&snap, &"AMZN".to_string(), "Robinhood", sale_qty, Date(20_000));
...
fn lot_picker_renders_as_a_table_with_a_running_total_in_the_footer_band() {
    ...
    assert!(s.contains("allocated"), "the running allocated N / M is in the footer band");
...
fn lot_picker_footer_holds_the_est_preview_never_over_a_data_row() {
    ...
    let last_lot_row = lines.iter().rposition(|l| l.contains("take [")).expect("a lot data row");
    let preview_row = lines.iter().position(|l| l.contains("est. gain")).expect("the preview row");
    assert!(preview_row > last_lot_row, "the preview is in the footer band beneath the data rows");
}
...
fn lot_picker_inline_error_renders_in_the_footer_band_not_over_a_data_row() {
    ...
    model
        ...
        .unwrap()
        .form
        .set_error(0, &InlineError::Field("allocation exceeds remaining".to_string()));
    let rt = empty_runtime();
    let s = render_string(&model, &rt, 100, 24);
    assert!(s.contains("allocation exceeds remaining"), "the inline error renders in the footer band");
    ...
    assert!(err_row > last_lot_row, "the error is beneath the data rows, never over one");
}
...
fn tab_and_shift_tab_move_between_fields_wrapping() {
    ...
    let mut model = open_buy_model();
    assert_eq!(model.entry_top().unwrap().form.focus, 0, "focus starts on the first field");
    ...
    assert_eq!(model.entry_top().unwrap().form.focus, n - 1, "shift-tab wraps to the last field");
    // tab from the last wraps to the first.
    model.entry_focus_next();
    assert_eq!(model.entry_top().unwrap().form.focus, 0, "tab wraps to the first field");
}
...
fn enter_advances_fields_then_submits_on_the_last_field() {
    ...
    for i in 0..n - 1 {
        ...
        assert_eq!(action, FormAction::Consumed, "enter on field {i} advances, not submits");
    }
    // Now on the last field.
    assert!(model.entry_top().unwrap().form.on_last_field());
    let action = model.entry_enter();
    assert_eq!(action, FormAction::Submit, "enter on the last field submits");
}
...
fn esc_with_a_dirty_field_requests_confirm_discard_without_popping() {
    ...
    let action = model.entry_escape();
    assert_eq!(action, FormAction::Consumed, "esc from text-input returns to field-nav, not cancel");
    assert_eq!(model.entry_top().unwrap().form.input_mode, InputMode::FieldNav);

    // Now esc in field-nav with the dirty field requests a confirm-discard.
    assert!(model.entry_top().unwrap().form.is_dirty(), "the edited field is dirty");
    let action = model.entry_escape();
    assert_eq!(action, FormAction::ConfirmDiscard, "a dirty esc requests confirm-discard");
    assert!(model.entry_top().is_some(), "the context is NOT popped before confirmation");

    // On confirmation the shell discards.
    model.entry_discard();
    assert!(model.entry_top().is_none(), "confirmation discards the context");
...
fn a_focused_field_enters_text_input_mode_suppressing_global_accelerators() {
    ...
    // (TUI-ENTRY-FLOW-008)
    let mut model = open_buy_model();
    assert!(!model.entry_in_text_input(), "starts in field-navigation mode (accelerators live)");

    model.entry_enter_field();
    assert!(model.entry_in_text_input(), "the focused field is now in text-input mode");
    ...
    assert!(!hint.contains("[enter] submit"), "global accelerators are suppressed in text-input mode");
    assert!(hint.contains("[esc] field"), "only the field's own escape applies in text-input mode");

    // esc returns to field-navigation mode (NOT a flow cancel).
    let action = model.entry_escape();
    assert_eq!(action, FormAction::Consumed, "esc in text-input returns to field-nav, not cancel");
    assert!(!model.entry_in_text_input(), "back in field-navigation mode");
...
fn the_active_context_is_the_top_stack_frame_and_esc_pops_it() {
    ...
    assert_eq!(model.entry_top().unwrap().form.kind, FlowKind::Split, "the top frame is active");

    let action = model.entry_escape();
    assert_eq!(action, FormAction::Cancel);
    assert_eq!(model.entry_top().unwrap().form.kind, FlowKind::Buy, "esc pops back to the parent context");
    assert_eq!(model.mode, Mode::Entry, "a remaining context keeps Entry mode");
}
...
fn typing_into_a_field_edits_its_value_and_marks_it_dirty() {
    ...
    assert!(!model.entry_top().unwrap().form.is_dirty(), "typing is suppressed outside text-input mode");
    ...
    assert!(f.value.ends_with("ZM"), "the typed chars land in the field value: {}", f.value);
    assert!(f.dirty, "an edited field is dirty");

    model.entry_top_mut().unwrap().form.backspace();
    assert!(model.entry_top().unwrap().form.fields[0].value.ends_with('Z'));
}
...
fn form_builders_seed_fields_for_buy_vest_sell_in_tab_order() {
    ...
    assert!(bf.fields.iter().any(|f| f.label == "Fees"), "Buy has a Fees field");

    let vest = VestForm::with_defaults(&rt, "V1".to_string(), "AMZN", &platforms, &aliases);
    let vf = FormModel::for_vest(&vest);
    assert!(vf.fields.iter().any(|f| f.label == "FMV/share"), "Vest has FMV/share");
    assert!(!vf.fields.iter().any(|f| f.label == "Fees"), "Vest has no Fees field");

    let sell = SellForm::with_defaults(&rt, "S1".to_string(), "AMZN", &residency, &platforms, &aliases);
    let sf = FormModel::for_sell(&sell);
    assert!(sf.fields.iter().any(|f| f.label == "Accrues to"), "Sell carries accrues-to-state");
