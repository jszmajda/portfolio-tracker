...
fn submit_revalidates_live_and_the_specific_kernel_error_rerenders_inline() {
    ...
    match &phase {
        Phase::Rejected(entry::InlineError::Ledger(e)) => {
            assert_eq!(*e, ledger_core::LedgerError::InsufficientShares, "the specific kernel error re-renders inline");
        }
        other => panic!("expected an inline kernel rejection, got {other:?}"),
    }
    assert_ne!(phase, Phase::Confirmed, "nothing confirmed on a submit-time disagreement");
    assert!(!matches!(phase, Phase::Retry(_)), "a kernel disagreement is inline, not a write-verify retry");
    assert!(rt.ledger_log.is_empty(), "a submit-time rejection writes nothing");
    // The inline error renders the verified error's words beside the field.
    if let Phase::Rejected(ie) = &phase {
        assert!(ie.text().contains("InsufficientShares"), "the inline slot shows the kernel error words: {}", ie.text());
    }
}
...
fn submit_confirmed_durable_clears_the_composer() {
    ...
    let candidate = a_buy().compose();
    let phase = entry::submit_ledger(&mut rt, &candidate);
    assert_eq!(phase, Phase::Confirmed, "a confirmed-durable submit clears the composer");
...
fn write_failure_returns_control_with_retry_and_retry_is_idempotent() {
    ...
    assert_eq!(rt.ledger_log.len(), 1, "retry lands exactly once (idempotent)");
}
...
fn lock_held_fails_non_destructively_without_queuing() {
    ...
    assert!(notice.contains("lock held"), "warn notice names lock held: {notice}");
    assert!(notice.contains('\u{26A0}'), "the warn glyph travels with the notice");
}
...
fn defaults_reduce_typing_date_today_residency_platform_alias() {
    ...
    assert_eq!(sell.accrues_to_state, Some("DC".to_string()), "accrues_to_state from residency_on(sale_date)");
    assert_eq!(sell.platform, "Robinhood", "platform from the first config suggestion");
    assert_eq!(sell.symbol, "BRK.B", "the typed symbol resolves through the alias table");
    ...
    let buy = tui::entry::BuyForm::with_defaults(&rt, "L1".to_string(), "AMZN", &platforms, &aliases);
    ...
    assert!(!buy.confirmed_new_symbol, "a defaulted Buy starts unconfirmed for the new-symbol guard");
}
