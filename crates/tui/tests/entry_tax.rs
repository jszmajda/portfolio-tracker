...
fn pay_covers_moved_accruals_with_amount_independent_and_advisory_balance() {
    ...
    let balanced = PayAdvisory {
        covered_cents: Cents(1_210_000),
        paid_cents: Cents(1_210_000),
    };
    assert!(balanced.balanced());
    assert!(balanced.text().contains('\u{2713}'), "✓ on balance");

    let partial = PayAdvisory {
        covered_cents: Cents(1_210_000),
        paid_cents: Cents(1_000_000),
    };
    assert!(!partial.balanced(), "a partial payment is allowed");
    assert_eq!(partial.delta(), Cents(-210_000), "the delta surfaces");
    assert!(
        partial.text().contains('\u{0394}'),
        "the delta is shown, not gated"
    );
}
...
fn override_replaces_the_computed_amount_with_an_absolute_amount_and_reason() {
    let ev = entry::compose_override(
        key("S1", "L1"),
        Cents(5_000),
        "negotiated settlement".to_string(),
    );
    match ev.kind {
...
fn batch_snapshots_at_confirm_and_revalidates_at_submit_flagging_vanished_or_repriced() {
    ...
    let current = vec![accrual(
        "S1",
        "L1",
        210_000,
        AccrualState::Allocated {
            account_label: "R".to_string(),
        },
    )];
    ...
    assert!(deltas
        .iter()
        .any(|d| matches!(d, BatchDelta::Repriced { key: k, .. } if k == &key("S1", "L1"))));
    assert!(deltas
        .iter()
        .any(|d| matches!(d, BatchDelta::Vanished(k) if k == &key("S1", "L2"))));
...
fn tax_accrual_lifecycle_allocate_then_move_then_pay_confirms_durable_through_the_write_loop() {
    ...
    assert_eq!(
        submit_tax(&mut rt, &allocate),
        Phase::Confirmed,
        "Allocate confirms durable"
    );
    assert_eq!(
        rt.tax_log.len(),
        1,
        "the Allocate landed in the accepted log"
    );
    ...
    assert_eq!(
        submit_tax(&mut rt, &mv),
        Phase::Confirmed,
        "Move on Allocated confirms durable"
    );
    ...
    assert_eq!(
        submit_tax(&mut rt, &pay),
        Phase::Confirmed,
        "Pay on a Moved accrual confirms durable"
    );
    ...
    assert!(
        matches!(submit_tax(&mut rt, &double), Phase::Rejected(_)),
        "a double Pay is rejected live"
    );
}
...
fn pay_covering_a_mismatched_jurisdiction_or_year_is_rejected_live() {
    ...
    assert_eq!(
        submit_tax(
            &mut rt,
            &entry::compose_allocate(k.clone(), "R".to_string())
        ),
        Phase::Confirmed
    );
    assert_eq!(
        submit_tax(
            &mut rt,
            &entry::compose_move(k.clone(), Cents(240_000), SALE_2026)
        ),
        Phase::Confirmed
    );
    ...
    match submit_tax(&mut rt, &mismatched) {
        Phase::Rejected(entry::InlineError::Tax(e)) => {
            assert_eq!(
                e,
                tax::TaxError::PayCoverMismatch,
                "the jurisdiction/year mismatch is the live rejection"
            );
        }
...
fn batch_submit_revalidates_live_at_submit_and_a_vanished_selection_prevents_the_commit() {
    ...
    let snapshot = BatchSnapshot {
        selected: vec![
            (k1.clone(), Some(Cents(240_000))),
            (k2.clone(), Some(Cents(96_000))),
        ],
    };
    ...
    match submit_tax_batch(
        &mut stale_rt,
        &snapshot,
        &[allocate1.clone(), allocate2.clone()],
    ) {
        BatchSubmit::Stale(deltas) => {
            assert!(deltas
                .iter()
                .any(|d| matches!(d, BatchDelta::Repriced { key: k, .. } if k == &k1)));
            assert!(deltas
                .iter()
                .any(|d| matches!(d, BatchDelta::Vanished(k) if k == &k2)));
        }
    ...
    match submit_tax_batch(&mut clean_rt, &snapshot, &[allocate1, allocate2]) {
        BatchSubmit::Submitted(phases) => {
            ...
            assert!(
                phases.iter().all(|p| *p == Phase::Confirmed),
                "a clean batch submits each event durably"
            );
        }
...
fn tax_submit_lock_held_and_write_failed_branches_preserve_the_entry() {
    ...
    let mut locked = tax_runtime("S1", "L1", 10_000_00).with_behavior(SubmitBehavior::LockHeld {
    ...
    let mut failed =
        tax_runtime("S1", "L1", 10_000_00).with_behavior(SubmitBehavior::VerifyMismatch);
    assert_eq!(
    ...
    assert!(
        failed.tax_log.is_empty(),
        "a write failure leaves no partial write"
    );
}
