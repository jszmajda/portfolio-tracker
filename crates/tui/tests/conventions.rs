...
fn freshness_offline_marks_figures_stale_with_the_last_quote_epoch_never_live() {
    ...
    let q = Qualifier::Stale {
        quote_epoch: "20000".to_string(),
    };
    assert!(
        q.marker().contains('\u{27F2}'),
        "the ⟲ stale glyph travels with the figure"
    );
...
fn degradation_distinguishes_unpriced_from_tax_degraded_with_distinct_words() {
    ...
    assert!(
        !tax_deg.marker().contains("unpriced"),
        "a tax-degraded figure is never mislabelled unpriced"
    );
...
fn estimates_carry_est_and_degrade_to_stale_brackets_and_no_brackets_wording() {
    assert_eq!(
        theme::estimate_qualifier(config::BracketState::Verified).marker(),
        "[est]"
    );
    assert_eq!(
...
fn integrity_error_blocks_the_screen_loudly_refusing_to_render_derived_numbers() {
    ...
    assert!(
        s.contains('\u{2717}'),
        "the ✗ integrity glyph is shown loudly"
    );
    assert!(
        s.to_uppercase().contains("INTEGRITY"),
        "a blocking message is shown"
    );
    assert!(
        !s.contains("$261.26") && !s.contains("[est]"),
        "no derived numbers are rendered"
    );

    // The other two integrity causes carry their messages.
    assert!(tui::port::IntegrityError::BadCredentials
        .message()
        .contains("credentials"));
    assert!(tui::port::IntegrityError::UnreadableCache
        .message()
        .contains("cache"));
}
...
fn confirmations_restate_what_will_be_written_for_gated_flows_only() {
    ...
    assert!(
        notice.contains("reprices 3"),
        "the tax-rule confirm restates the blast radius"
    );
}
...
fn color_degrades_truecolor_to_256_to_16_to_none_keeping_glyphs() {
    ...
    assert_eq!(
        tc.color(Role::Accent),
        Color::Rgb(0xD4, 0xA8, 0x2C),
        "the gilt accent hex"
    );
    assert_eq!(
        tc.color(Role::Gain),
        Color::Rgb(0x6F, 0xA8, 0x6B),
        "sage gain"
    );
    ...
    match i256.color(Role::Accent) {
        Color::Indexed(i) => assert!(
            (16..=231).contains(&i),
            "the accent degrades to a cube index, got {i}"
        ),
    ...
    match i256.color(Role::FgDim) {
        Color::Indexed(i) => assert!(
            i >= 232 || (16..=231).contains(&i),
            "fg-dim maps to a 256 index, got {i}"
        ),
...
fn lifecycle_stepper_motif_colours_the_minting_ramp_and_renders_distinct_settled_orphan() {
    ...
    use ratatui::style::Color;
    assert_eq!(
        pal.lifecycle_color(LifecycleStop::Allocated),
        Color::Rgb(0x5F, 0xA8, 0x9E)
    );
    assert_eq!(
        pal.lifecycle_color(LifecycleStop::Moved),
        Color::Rgb(0xD4, 0xA8, 0x2C)
    );
    assert_eq!(
        pal.lifecycle_color(LifecycleStop::Paid),
        Color::Rgb(0x6F, 0xA8, 0x6B)
    );
    ...
    assert_eq!(
        theme::stepper(&StepperState::Lifecycle(LifecycleStop::Moved)),
        "◉◉◉○ Moved"
    );
    assert_eq!(
        theme::stepper(&StepperState::Lifecycle(LifecycleStop::Paid)),
        "◉◉◉◉ Paid"
    );
...
fn sparkline_motif_renders_ramp_and_tints_by_net_direction() {
    ...
    assert!(
        rising.ends_with('\u{2588}'),
        "the latest (max) cell is brightest"
    );
...
fn view_money_is_whole_dollar_with_a_compact_glance_form_and_per_share_cents() {
    ...
    assert_eq!(
        theme::money_whole(Cents(50)),
        "$0",
        "half-to-even: 50¢ → the even 0"
    );
    assert_eq!(
        theme::money_whole(Cents(150)),
        "$2",
        "half-to-even: $1.50 → the even 2"
    );
    ...
    assert_eq!(
        theme::money_compact(Cents(99_996)),
        "$1.0k",
        "$999.96 promotes to k"
    );
    assert_eq!(
        theme::money_compact(Cents(99_996_000)),
        "$1.00m",
        "$999,960 promotes to m"
    );
...
fn status_line_lock_glyph_is_catalogued_and_renders_held_vs_free() {
    ...
    assert!(
        status.text().contains(theme::GLYPH_LOCK_FREE),
        "the free-lock glyph renders"
    );
    ...
    assert!(
        held.contains(theme::GLYPH_LOCK_HELD),
        "the held-lock glyph renders"
    );
    assert!(
        held.contains("lock held"),
        "the redundant word travels with the glyph"
    );
}
...
fn masthead_and_status_line_motifs_render_in_the_buffer() {
    ...
    assert!(
        ...
        "the status line names the current screen"
    );
}
