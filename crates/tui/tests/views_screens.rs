...
use tui::testkit::{buy, flat_federal_ctx, render_string, replay, series_point, FakeRuntime, ViewBuilder};
...
fn positions_render_per_symbol_value_net_est_delta_share_and_sparkline() {
    ...
    assert!(amzn_line.text().contains("[est]"), "the post-tax net carries [est]: {}", amzn_line.text());
    assert_eq!(
...
fn grouping_toggle_repivots_by_platform_with_full_group_subtotals_under_filter() {
    ...
    assert!(rows.iter().any(|r| r.label == "Robinhood"), "re-pivoted by platform");
    ...
    assert_eq!(robinhood.market_value, Some(Cents(120_000)), "the subtotal reflects the full group");
    ...
    assert!(!plat_rows.iter().any(|r| r.label == "Schwab"), "a platform filter narrows the visible groups");
    assert_eq!(
        plat_rows.iter().find(|r| r.label == "Robinhood").unwrap().market_value,
        Some(Cents(120_000)),
...
fn priced_coverage_caveat_on_header_and_share_na_distinct_from_degraded() {
    ...
    assert_eq!(caveat.degraded_count, 1, "PLTR counted in the priced-coverage caveat");
    assert!(!caveat.shares_na, "AMZN is priced → shares are well-defined (not n/a)");

    let nav = NavState::new(Screen::Positions);
    let (rows, _c, _h) = views::positions_rows(&view, &nav);
    let pltr = rows.iter().find(|r| r.label == "PLTR").unwrap();
    assert!(pltr.degraded, "PLTR is degraded (the ‡ marker, never 0)");
    assert!(!pltr.share_na, "the degraded ‡ marker is distinct from share n/a");
...
fn open_lots_render_per_lot_fields_and_are_the_drill_down_target() {
    ...
    assert_eq!(
        views::drill_target(&Screen::Positions, &views::RowIdentity::Symbol("AMZN".to_string())),
...
fn history_renders_chart_plus_table_with_gap_blank_and_incomplete_tinted() {
    ...
    let view = ViewBuilder::new(snap)
        .history_point(series_point(TradingDayKey(Date(100)), 1_000_00, &[("AMZN", 1_000_00)], false))
        .calendar_day(TradingDayKey(Date(101))) // a gap
        .history_point(series_point(TradingDayKey(Date(102)), 1_200_00, &[("AMZN", 1_200_00)], true))
        .build();
    let nav = NavState::new(Screen::History);
    let hv = views::history_view(&view, &nav);
    assert!(matches!(hv.state, views::HistoryState::Normal));
    assert!(matches!(hv.cells[0], views::HistoryCell::Value { .. }), "day A is a value");
    assert!(matches!(hv.cells[1], views::HistoryCell::Gap { .. }), "day B is an explicit gap (blank, not interpolated)");
    assert!(matches!(hv.cells[2], views::HistoryCell::Incomplete { .. }), "day C is a ‡-tinted incomplete cell");

    // The rendered gap line shows the gap convention; the incomplete shows ‡.
    let lines = screen_lines(&view, &nav);
    assert!(lines.iter().any(|l| l.text().contains("gap")), "an in-band gap renders blank-ish");
    assert!(lines.iter().any(|l| l.text().contains('\u{2021}')), "an incomplete day carries ‡");
}
...
fn history_degenerate_states_render_explicitly() {
    ...
    assert!(matches!(views::history_view(&v0, &nav).state, views::HistoryState::NoHistory));
    ...
    let v1 = ViewBuilder::new(snap.clone())
        ...
        .build();
    assert!(matches!(views::history_view(&v1, &nav).state, views::HistoryState::SinglePoint));
    ...
    assert!(hv.sparkline.is_empty(), "the chart is suppressed when all-incomplete");
}
...
fn tax_renders_autosettled_and_orphan_states_distinctly() {
    ...
    let view = ViewBuilder::new(ledger_core::Snapshot::default())
        ...
        .orphans(vec![tax::OrphanWarning { accrual_key: orphan_key.clone(), kind: tax::OrphanKind::Move }])
        .build();
    let nav = NavState::new(Screen::TaxReserves);
    let (rows, _r, _h) = views::tax_rows(&view, &nav);

    let settled_row = rows.iter().find(|r| r.key.sale_id == "Sde").unwrap();
    assert!(matches!(settled_row.stepper, tui::theme::StepperState::AutoSettled));
    assert!(!settled_row.actionable, "a de-minimis auto-settled accrual is excluded from actionable selection");
    assert!(tui::theme::stepper(&settled_row.stepper).contains("settled"));

    let orphan_row = rows.iter().find(|r| &r.key == &orphan_key).unwrap();
    assert!(matches!(orphan_row.stepper, tui::theme::StepperState::Orphaned));
...
fn realized_renders_calendar_years_labelled_calendar_with_provenance() {
    ...
    let log = vec![
        ...
        ledger_core::LedgerEvent {
            ...
            kind: ledger_core::LedgerEventKind::Sell {
                ...
                fees_cents: Cents(0),
                lot_refs: vec![ledger_core::LotRef { lot_id: "L-A".to_string(), qty: common::sh(100) }],
    ...
    assert!(rows[0].label.starts_with("calendar "), "labelled calendar (distinct from tax's quarterly)");
...
fn rendered_buffer_carries_the_masthead_and_status_line() {
    ...
    let rt: FakeRuntime = FakeRuntime::new(common::two_position_view(), flat_federal_ctx(common::YEAR, 220_000));
    let model = Model::new();
    let s = render_string(&model, &rt, 90, 20);
    assert!(s.contains("L E D G E R"), "the masthead renders");
    assert!(s.contains("connected") || s.contains("stale"), "the status line renders the connection");
}
...
fn positions_row_carries_shares_price_and_pretax_unrealized_columns() {
    ...
    let (rows, _c, _h) = views::positions_rows(&view, &nav);
    let amzn = rows.iter().find(|r| r.label == "AMZN").unwrap();
    assert_eq!(amzn.shares, Some(common::sh(620)), "the share count rides the row");
    assert_eq!(amzn.price, Some(Cents(26_126)), "the mark price rides the row");
    ...
    assert!(amzn_line.contains('\u{25B2}'), "pre-tax unrealized carries ▲: {amzn_line}");
    assert_eq!(
    ...
    assert!(amzn_line.matches('│').count() >= 5, "│-ruled columns: {amzn_line}");
    assert_eq!(
        amzn.span_role("│"),
...
fn positions_render_the_total_value_and_net_post_tax_summary_band_with_double_rule() {
    ...
    let nav = NavState::new(Screen::Positions);
    let lines = screen_lines(&view, &nav);
    let total = lines.iter().find(|l| l.text().starts_with("TOTAL VALUE")).expect("TOTAL VALUE band");
    ...
    assert!(total.text().contains("$184,563"), "the whole-book total: {}", total.text());
    assert!(!total.text().contains("$184.6k"), "the band never abbreviates: {}", total.text());
    ...
    assert_eq!(total.span_role("TOTAL VALUE"), Some(tui::theme::Role::FgFaint));
    let figure = total.spans.iter().find(|s| s.text.contains("$184,563")).unwrap();
    assert_eq!(figure.role, tui::theme::Role::Fg);
    assert!(figure.bold, "the figure beside a tracked-caps label is bold");
    let net = lines.iter().find(|l| l.text().starts_with("NET (POST-TAX)")).expect("NET band");
    assert!(net.text().contains("[est]"), "the post-tax net is an estimate: {}", net.text());
    assert_eq!(
        net.span_role("[est]"),
        Some(tui::theme::Role::Estimate),
        "the [est] qualifier renders in the estimate role"
    ...
    assert!(lines.iter().any(|l| l.text().starts_with('\u{2550}')), "a ═ rule renders");
    // The band precedes the rows.
    let band_idx = lines.iter().position(|l| l.text().starts_with("TOTAL VALUE")).unwrap();
    let row_idx = lines.iter().position(|l| l.text().contains("AMZN")).unwrap();
...
fn positions_summary_band_never_renders_zero_for_an_unpriced_book() {
    ...
    let nav = NavState::new(Screen::Positions);
    let lines = screen_lines(&view, &nav);
    let total = lines.iter().find(|l| l.text().starts_with("TOTAL VALUE")).expect("band renders");
    assert!(!total.text().contains("$0"), "never a fabricated zero: {}", total.text());
    assert!(total.text().contains('\u{2021}'), "the ‡ marker travels with the figure: {}", total.text());
}
...
fn focus_caret_marks_only_the_focused_row_over_the_bg_focus_token() {
    ...
    let lines = screen_lines(&view, &nav);
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap().text();
    let googl = lines.iter().find(|l| l.text().contains("GOOGL")).unwrap().text();
    assert!(googl.starts_with('\u{258E}'), "the focused row carries the ▎ caret: {googl}");
    assert!(!amzn.starts_with('\u{258E}'), "an unfocused row has a blank gutter: {amzn}");
...
fn header_band_carries_the_right_aligned_as_of_and_stale_glyph_when_offline() {
    ...
    assert!(first.trim_end().ends_with("as-of 2024-10-04"), "right-aligned calendar as-of: {first}");
    assert!(!first.contains("as-of 20000"), "never the raw key int: {first}");
    ...
    assert!(first2.contains('\u{27F2}'), "offline carries the ⟲ stale glyph: {first2}");
    assert!(first2.contains("as-of 2024-10-04"), "with the last calendar as-of: {first2}");
}
...
fn a_book_with_no_priced_day_reads_no_priced_day_yet_never_zero() {
    ...
    let view = ViewBuilder::new(ledger_core::Snapshot::default()).no_priced_day().build();
    let rt = FakeRuntime::new(view.clone(), flat_federal_ctx(common::YEAR, 220_000));
    let model = Model::new();
    let s = render_string(&model, &rt, 100, 20);
    let first = s.lines().next().unwrap();
    assert!(first.contains("no priced day yet"), "the masthead names the state: {first}");
    assert!(!first.contains("as-of 0") && !first.contains("as-of 20000"), "never a key int: {first}");

    let status = tui::StatusLine::from_view(&view, tui::Mode::Views, &Screen::Positions);
    assert!(status.text().contains("no priced day yet"), "the status line too: {}", status.text());
}
...
fn status_line_carries_the_updated_wall_clock_and_omits_it_when_unknown() {
    ...
    let view = ViewBuilder::new(ledger_core::Snapshot::default()).updated("16:42").build();
    let status = tui::StatusLine::from_view(&view, tui::Mode::Views, &Screen::Positions);
    assert!(status.text().contains("updated 16:42"), "the wall-clock renders: {}", status.text());
    ...
    assert!(!status.text().contains("updated"), "no fabricated time: {}", status.text());
    ...
    assert!(bottom.contains("updated 16:42"), "the status line renders it: {bottom}");
}
...
fn open_lots_line_renders_acquire_date_basis_and_tracking_code() {
    ...
    let nav = NavState::new(Screen::OpenLots);
    let lines = screen_lines(&view, &nav);
    let line = lines.iter().find(|l| l.text().contains("L-A")).unwrap().text();
    ...
    assert!(line.contains("basis $800"), "the lot basis rendered (100 sh @ $8): {line}");
    assert!(!line.contains("$800.00"), "the basis drops its cents: {line}");
    assert!(line.contains("$8.00/sh"), "basis/share keeps cents: {line}");
    assert!(line.contains('\u{2014}'), "an absent tracking code renders the — dash: {line}");
...
fn tax_rows_group_by_jurisdiction_year_and_colour_by_the_minting_ramp() {
    ...
    alloc.state = tax::AccrualState::Allocated { account_label: "Reserve".into() };
    ...
    assert_eq!(dots_role("S1/L1"), tui::theme::Role::FgFaint, "Accrued dots render faint");
    assert_eq!(dots_role("S2/L2"), tui::theme::Role::Estimate, "Allocated dots render verdigris");
    assert_eq!(dots_role("S3/L3"), tui::theme::Role::Accent, "Moved dots render gilt");
    assert_eq!(dots_role("S4/L4"), tui::theme::Role::Gain, "Paid dots render sage");
    assert_eq!(
...
fn history_chart_is_tinted_by_net_direction_not_chrome() {
    ...
    assert_eq!(first.role, tui::theme::Role::Gain, "a rising series tints gain");
    ...
    assert!(tick.bold, "the latest captured column is brightened (the today tick)");
    assert_eq!(tick.role, tui::theme::Role::Gain, "the tick keeps the direction tint");
    ...
    let first = row.spans.iter().find(is_ramp).unwrap();
    assert_eq!(first.role, tui::theme::Role::Loss, "a falling series tints loss");
    assert!(
...
fn realized_and_open_lots_render_their_dedicated_empty_states() {
    ...
    let log = vec![
        ...
        ledger_core::LedgerEvent {
            ...
            kind: ledger_core::LedgerEventKind::Sell {
                ...
                unit_price_cents: Cents(9_989),
                fees_cents: Cents(0),
                lot_refs: vec![ledger_core::LotRef { lot_id: "L-A".to_string(), qty: common::sh(10) }],
                accrues_to_state: None,
                platform: "Robinhood".to_string(),
                tracking_code: None,
            },
    ...
    assert!(
        lines.iter().any(|l| l.text() == "no open lots — every lot has been fully sold"),
    ...
    assert!(
        lines.iter().any(|l| l.text() == "no realized gains yet — they appear when you sell a lot"),
...
fn screens_use_their_design_names_and_only_positions_shows_the_of_book_segment() {
    ...
    assert!(
        lines.iter().any(|l| l.text().starts_with("showing") && !l.text().contains("of book")),
...
fn positions_rows_carry_basis_and_gain_pct_of_basis() {
    ...
    assert_eq!(amzn.basis, Cents(31_000_00), "the total cost basis rides the row");
    assert_eq!(amzn.gain_pct_ppm, Some(4_225_200), "the unr-% of basis from reports");
    assert!(!amzn.gain_pct_na);

    let lines = screen_lines(&view, &nav);
    let line = lines.iter().find(|l| l.text().contains("AMZN")).unwrap().text();
...
fn gain_pct_renders_na_on_zero_basis_and_dash_when_degraded() {
    ...
    assert!(zero.gain_pct_na && zero.gain_pct_ppm.is_none(), "basis ≤ 0 → n/a");
    let pltr = rows.iter().find(|r| r.label == "PLTR").unwrap();
    ...
    let lines = screen_lines(&view, &nav);
    let zero_line = lines.iter().find(|l| l.text().contains("ZERO")).unwrap().text();
    assert!(zero_line.contains("n/a"), "the zero-basis gain-% cell reads n/a: {zero_line}");
    assert!(!zero_line.contains('\u{2021}'), "n/a is distinct from the degraded ‡: {zero_line}");
    let pltr_line = lines.iter().find(|l| l.text().contains("PLTR")).unwrap().text();
    assert!(pltr_line.contains("$500"), "basis renders even on a degraded row: {pltr_line}");
}
...
fn day_change_uses_the_latest_prior_capture_with_per_symbol_honesty() {
    ...
    let view = ViewBuilder::new(snap)
        ...
        .history_point(series_point(TradingDayKey(Date(0)), 9_999_00, &[("AMZN", 9_999_00)], false))
        ...
        .history_point(series_point(TradingDayKey(Date(19_999)), 2_500_00, &[("AMZN", 2_500_00)], true))
        .build();
    let nav = NavState::new(Screen::Positions);
    let (rows, _c, _h) = views::positions_rows(&view, &nav);
    let amzn = rows.iter().find(|r| r.label == "AMZN").unwrap();
    // Current 10 sh @ $261.26 = $2,612.60 vs the latest prior's $2,500.00.
    assert_eq!(amzn.day_delta, Some(Cents(112_60)), "vs the LATEST prior capture");
    assert_eq!(amzn.day_delta_ppm, Some(45_040), "the % relative to the prior value");
    ...
    let lines = screen_lines(&view, &nav);
    let line = lines.iter().find(|l| l.text().contains("AMZN")).unwrap().text();
    assert!(line.contains("+$113"), "the $ day change renders (compact): {line}");
    assert!(line.contains("(+4.5%)"), "the % rides the same delta cell: {line}");
    let fline = lines.iter().find(|l| l.text().contains("PENNY")).unwrap().text();
    assert!(fline.contains('\u{2014}'), "the PENNY day-change cell holds the dash: {fline}");
}
...
fn day_change_is_a_dash_until_a_prior_capture_exists() {
    ...
    let only_today = ViewBuilder::new(snap.clone())
        .mark("AMZN", 26_126, Date(20_000))
        .estimate("AMZN", common::est("AMZN", 10_00, 2_00))
        .history_point(series_point(TradingDayKey(Date(20_000)), 2_612_60, &[("AMZN", 2_612_60)], false))
        .build();
    let (rows, _c, _h) = views::positions_rows(&only_today, &nav);
    assert_eq!(rows[0].day_delta, None, "today's capture alone is not a prior");
    let lines = screen_lines(&only_today, &nav);
    let line = lines.iter().find(|l| l.text().contains("AMZN")).unwrap().text();
    assert!(line.contains('\u{2014}'), "the dash holds the day-change cell: {line}");
...
fn summary_band_carries_the_realized_ytd_line_for_the_current_tax_year() {
    ...
    assert!(ytd.text().contains("+$4,989"), "only the 2026 sale, whole dollars: {}", ytd.text());
    assert!(!ytd.text().contains("9,978"), "the 2025 sale is excluded: {}", ytd.text());
    assert!(ytd.text().contains('\u{25B2}'), "the gain carries ▲: {}", ytd.text());
    assert_eq!(ytd.span_role("\u{25B2}"), Some(tui::theme::Role::Gain));
    assert_eq!(ytd.span_role("REALIZED"), Some(tui::theme::Role::FgFaint), "tracked-caps label");
    // The band line sits above the double-entry rule with the other band lines.
    let ytd_idx = lines.iter().position(|l| l.text().starts_with("REALIZED")).unwrap();
    let rule_idx = lines.iter().position(|l| l.text().starts_with('\u{2550}')).unwrap();
...
fn realized_ytd_renders_exact_zero_when_nothing_sold_and_survives_an_unpriced_book() {
    ...
    assert!(ytd.text().contains("\u{00B7} $0"), "the exact flat zero: {}", ytd.text());
    assert!(!ytd.text().contains('\u{2014}'), "never a dash: {}", ytd.text());
    ...
    assert!(
        lines.iter().any(|l| l.text().starts_with("REALIZED 2026 YTD")),
...
fn history_renders_a_multi_row_chart_with_axis_labels_and_date_span() {
    ...
    let chart: Vec<&tui::ScreenLine> =
        lines.iter().filter(|l| l.text().contains('\u{2524}')).collect();
    assert!(chart.len() >= 4, "a multi-row chart, got {} rows", chart.len());
    ...
    assert!(
        lines.iter().any(|l| l.text().contains("day 100") && l.text().contains("day 103")),
    ...
    let ramps = cells.iter().filter(|c| tui::theme::SPARK_RAMP.contains(c)).count();
    assert_eq!(ramps, 3, "three captured columns on the bottom row: {data:?}");
    assert_eq!(cells[2], ' ', "the gap day renders a blank column: {data:?}");
    let is_ramp = |s: &&tui::Seg| s.text.chars().any(|c| tui::theme::SPARK_RAMP.contains(&c));
    let tick = bottom.spans.iter().rev().find(is_ramp).expect("a data cell");
    assert!(tick.bold, "the latest captured column is bold (the today tick)");
    assert_eq!(tick.role, tui::theme::Role::Gain, "direction tint");
    assert!(
        chart.iter().any(|l| l
            .spans
            .iter()
            .any(|s| s.role == tui::theme::Role::Degraded
...
fn position_figures_carry_gain_loss_roles_and_net_carries_the_estimate_role() {
    ...
    let view = ViewBuilder::new(snap)
        .mark("AMZN", 26_126, Date(20_000))
        .estimate("AMZN", common::est("AMZN", 10_00, 2_00))
        .history_point(sp(TDK(Date(19_998)), 3_000_00, &[("AMZN", 3_000_00)], false))
        .build();
    let nav = NavState::new(Screen::Positions);
    let lines = screen_lines(&view, &nav);
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    ...
    assert_eq!(pct.role, tui::theme::Role::Gain, "gain-% rides the gain role: {pct:?}");
    ...
    assert_eq!(net.role, tui::theme::Role::Estimate, "the net figure is estimate-tinted");

    // The summary band's NET (POST-TAX) figure is estimate-tinted too.
    let band = lines.iter().find(|l| l.text().starts_with("NET (POST-TAX)")).unwrap();
...
fn net_equals_market_value_when_price_sits_below_the_fmv_basis() {
    ...
    let (rows, _c, _h) = views::positions_rows(&view, &nav);
    let amzn = rows.iter().find(|r| r.label == "AMZN").unwrap();
    assert_eq!(amzn.market_value, Some(Cents(14_270_250)), "750 sh @ $190.27");
    assert_eq!(amzn.basis, Cents(15_000_000), "basis = FMV at vest: 750 × $200.00");
    assert_eq!(amzn.unrealized_pretax, Some(Cents(-729_750)), "a $7,297.50 loss");
    assert_eq!(
    ...
    assert!(line.text().contains("$142.7k"), "VALUE stays compact: {}", line.text());
}
...
fn realized_renders_ledger_columns_under_a_title_row_with_gain_loss_roles() {
    ...
    let title = lines.iter().find(|l| l.text().contains("PERIOD")).expect("the title row");
    for t in ["PERIOD", "PROCEEDS", "BASIS", "GAIN"] {
        assert!(title.text().contains(t), "the {t} title renders: {}", title.text());
    }
    assert!(title.spans.iter().all(|s| s.role == tui::theme::Role::FgFaint || s.text.trim().is_empty()),
        "the title row is tracked-caps faint: {title:?}");
    let rule_pos = |l: &tui::ScreenLine| -> Vec<usize> {
        l.text().chars().enumerate().filter(|(_, c)| *c == '│').map(|(i, _)| i).collect()
    };
    let y2025 = lines.iter().find(|l| l.text().contains("2025")).expect("the 2025 row");
    let y2026 = lines.iter().find(|l| l.text().contains("2026")).expect("the 2026 row");
    assert!(rule_pos(title).len() >= 3, "│-ruled columns: {}", title.text());
    assert_eq!(rule_pos(title), rule_pos(y2025), "the title aligns:\n{}\n{}", title.text(), y2025.text());
    assert_eq!(rule_pos(y2025), rule_pos(y2026), "the rows share one grid");
    // The title precedes the data rows.
    let ti = lines.iter().position(|l| l.text().contains("PERIOD")).unwrap();
    let ri = lines.iter().position(|l| l.text().contains("2025")).unwrap();
    assert!(ti < ri, "the title row sits above the data");

    // Each year's gain figure carries its own role with the redundant glyph.
    assert_eq!(y2025.span_role("\u{25BC}"), Some(tui::theme::Role::Loss), "{}", y2025.text());
    assert_eq!(y2026.span_role("\u{25B2}"), Some(tui::theme::Role::Gain), "{}", y2026.text());

    // Realized figures are glance money: the compact whole-dollar form.
    // (TUI-VIEW-REAL-004)
    assert!(y2026.text().contains("$999"), "proceeds compact ($998.90 → $999): {}", y2026.text());
    assert!(y2026.text().contains("+$499"), "gain compact: {}", y2026.text());
    assert!(!y2026.text().contains(".90"), "no cents on a glance figure: {}", y2026.text());
}
...
fn tax_amounts_and_reserves_render_exact_whole_dollars_never_abbreviated() {
    ...
    let row = lines.iter().find(|l| l.text().contains("S1/L1")).expect("the accrual row").text();
    assert!(row.contains("$2,400"), "the accrual amount in exact whole dollars: {row}");
    assert!(!row.contains("$2,400.00") && !row.contains("$2.4k"), "never cents, never compact: {row}");
    ...
    let res = lines
        ...
        .text();
    assert!(res.contains("accrued $24,968"), "exact whole-dollar reserve: {res}");
    assert!(res.contains("moved $19,950"), "exact whole-dollar moved: {res}");
    assert!(res.contains("shortfall $5,018"), "exact whole-dollar shortfall: {res}");
    assert!(!res.contains("$25.0k") && !res.contains(".00"), "never compact, never cents: {res}");
}
...
fn tax_accrual_rows_render_under_an_aligned_title_row() {
    ...
    let nav = NavState::new(Screen::TaxReserves);
    let lines = screen_lines(&view, &nav);
    let title = lines.iter().find(|l| l.text().contains("SALE/LOT")).expect("the title row");
    ...
    let row = lines.iter().find(|l| l.text().contains("S1/L1")).expect("the accrual row");
    let rule = |l: &tui::ScreenLine| l.text().chars().position(|c| c == '│');
    assert_eq!(rule(title), rule(row), "the title aligns:\n{}\n{}", title.text(), row.text());
    // The title precedes the group header + rows.
    let ti = lines.iter().position(|l| l.text().contains("SALE/LOT")).unwrap();
    let ri = lines.iter().position(|l| l.text().contains("S1/L1")).unwrap();
    assert!(ti < ri);
}
