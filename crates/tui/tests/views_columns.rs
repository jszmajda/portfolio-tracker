...
fn end_of(line: &ScreenLine, needle: &str) -> usize {
    let text = line.text();
    let byte = text.find(needle).unwrap_or_else(|| panic!("{needle} not in {text}"));
...
fn position_columns_size_from_the_data_and_stay_aligned_across_extreme_rows() {
    ...
    assert!(amzn_rules.len() >= 5, "the │-ruled columns render: {}", amzn.text());
    assert_eq!(rule_positions(penny), amzn_rules, "PENNY aligns:\n{}\n{}", amzn.text(), penny.text());
    assert_eq!(rule_positions(priv_row), amzn_rules, "PRIVHLD aligns:\n{}\n{}", amzn.text(), priv_row.text());
    ...
    assert!(amzn.text().contains("$162.0k"), "the six-figure value compacts: {}", amzn.text());
    assert!(penny.text().contains("$0.04"), "the PENNY price keeps cents: {}", penny.text());
}
...
fn the_unpriced_dash_sits_in_its_column_and_the_word_marks_the_row() {
    ...
    assert!(text.contains("PRIVHLD"), "the 7-char symbol renders whole: {text}");
    assert!(!text.contains('…'), "no ellipsis below the clamp: {text}");
    assert!(text.contains("24,150"), "the real share count renders: {text}");
    assert!(text.contains('\u{2014}'), "the — dash holds the unpriced figures: {text}");
    assert!(text.contains("‡ unpriced"), "the unpriced word marks the row: {text}");
    ...
    assert!(dash_pos.is_some(), "a dash sits inside the market-value column: {}", priv_row.text());
}
...
fn a_symbol_is_ellipsized_only_when_the_clamp_forces_it() {
    let log = vec![
        ...
        buy(2, 18_100, "L-L", "EXTREMELYLONGTICKER", 10, 5_000, "Robinhood"),
    ];
    ...
    let view = ViewBuilder::new(snap)
        ...
        .estimate("EXTREMELYLONGTICKER", common::est("EXTREMELYLONGTICKER", 10_00, 2_00))
        .build();
    let lines = screen_lines(&view, &NavState::new(Screen::Positions));
    let long = lines.iter().find(|l| l.text().contains("EXTREMELY")).expect("the long row");
    assert!(long.text().contains('…'), "over the clamp the symbol ellipsizes: {}", long.text());
    assert!(
    ...
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    assert_eq!(rule_positions(amzn), rule_positions(long), "the grid holds under the clamp");
}
...
fn open_lot_columns_size_from_the_data_and_align() {
    ...
    let a = lines.iter().find(|l| l.text().contains("L-A ")).expect("the AMZN lot");
    let z = lines.iter().find(|l| l.text().contains("L-PRIVHLD")).expect("the PRIVHLD lot");
    assert_eq!(
        rule_positions(a),
        rule_positions(z),
    ...
    assert!(z.text().contains("24,150"), "the share count groups thousands: {}", z.text());
    assert_eq!(
        end_of(z, "24,150"),
        end_of(a, "100"),
...
fn position_row_trend_strip_carries_direction_tint_a_today_tick_and_dim_dot_padding() {
    ...
    let view = ViewBuilder::new(snap)
        .mark("AMZN", 26_126, Date(20_000))
        .estimate("AMZN", common::est("AMZN", 10_00, 2_00))
        .history_point(series_point(TradingDayKey(Date(19_998)), 1_000_00, &[("AMZN", 1_000_00)], false))
        .history_point(series_point(TradingDayKey(Date(19_999)), 1_500_00, &[("AMZN", 1_500_00)], false))
        .build();
    let lines = screen_lines(&view, &NavState::new(Screen::Positions));
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    ...
    assert_eq!(strip.chars().count(), 12, "a fixed 12-cell strip: {:?}", strip);
    assert!(strip.ends_with("··········"), "10 dim dots pad the young series: {strip}");
    ...
    assert_eq!(tick.role, tui::theme::Role::Gain, "the tick keeps the rising tint");
    // The dot padding is faint chrome — never bold, never the direction tint.
    let dots = amzn.spans.last().expect("spans");
    assert!(dots.text.contains('·'), "the strip tail is the dot pad: {:?}", amzn.spans);
...
fn trend_strip_renders_honest_states_under_two_points_and_windows_to_twelve() {
    ...
    let mk = |points: &[(i32, i64)]| {
        ...
        let mut b = ViewBuilder::new(snap)
            ...
            .estimate("AMZN", common::est("AMZN", 10_00, 2_00));
        for (d, v) in points {
            b = b.history_point(series_point(TradingDayKey(Date(*d)), *v, &[("AMZN", *v)], false));
        }
    ...
    let lines = mk(&[]);
    assert_eq!(amzn_strip(&lines), "············", "zero captures → all dots");
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    assert!(
    ...
    assert_eq!(amzn_strip(&lines), "▄···········", "one capture → a mid-rung tick + 11 dots");
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    let tick = amzn.spans.iter().find(|s| s.bold && s.text == "▄").expect("the lone tick is bold");
    assert_eq!(tick.role, tui::theme::Role::Flat, "a single point has no trend — flat tint");

    // Fourteen captures: the strip windows to the most recent TWELVE — full
    // width, no dots, the oldest two days dropped.
    let pts: Vec<(i32, i64)> = (0..14).map(|i| (19_986 + i, 1_000_00 + (i as i64) * 10_00)).collect();
    let lines = mk(&pts);
    let strip = amzn_strip(&lines);
    assert_eq!(strip.chars().count(), 12, "windowed to 12: {strip}");
    assert!(!strip.contains('·'), "a full window has no dot padding: {strip}");
}
...
fn platform_group_rows_render_no_trend_strip() {
    ...
    let view = ViewBuilder::new(snap)
        .mark("AMZN", 26_126, Date(20_000))
        .estimate("AMZN", common::est("AMZN", 10_00, 2_00))
        .history_point(series_point(TradingDayKey(Date(19_999)), 1_000_00, &[("AMZN", 1_000_00)], false))
        .build();
    let mut nav = NavState::new(Screen::Positions);
    nav.grouping = tui::views::Grouping::ByPlatform;
    let lines = screen_lines(&view, &nav);
    let row = lines.iter().find(|l| l.text().contains("Robinhood")).unwrap();
    assert_eq!(strip_text(row), "", "no strip on a platform group row: {}", row.text());
}
...
fn basis_and_gain_pct_columns_size_from_the_data_and_align() {
    ...
    assert!(priv_row.text().contains("$241.5k"), "the six-figure basis compacts: {}", priv_row.text());
    assert_eq!(
    ...
    // columns in place.
    assert!(penny.text().contains("33.3%"), "PENNY gain %: {}", penny.text());
    assert!(amzn.text().contains("422.5%"), "AMZN gain %: {}", amzn.text());
    let amzn_rules = rule_positions(amzn);
    assert_eq!(rule_positions(penny), amzn_rules, "PENNY aligns:\n{}\n{}", amzn.text(), penny.text());
    assert_eq!(rule_positions(priv_row), amzn_rules, "PRIVHLD aligns:\n{}\n{}", amzn.text(), priv_row.text());
}
...
fn positions_title_row_is_faint_tracked_caps_and_aligns_with_its_columns() {
    ...
    for t in ["SYMBOL", "NAME", "SHARES", "PRICE", "VALUE", "BASIS", "UNREAL", "GAIN%", "NET", "DAY", "SHARE"] {
    ...
    let amzn = lines.iter().find(|l| l.text().contains("AMZN")).unwrap();
    let priv_row = lines.iter().find(|l| l.text().contains("PRIVHLD")).unwrap();
    assert_eq!(rule_positions(title), rule_positions(amzn), "title aligns:\n{}\n{}", title.text(), amzn.text());
    assert_eq!(rule_positions(title), rule_positions(priv_row), "title aligns:\n{}\n{}", title.text(), priv_row.text());
    ...
    let title_idx = lines.iter().position(|l| l.text().contains("SYMBOL")).unwrap();
    let row_idx = lines.iter().position(|l| l.text().contains("AMZN")).unwrap();
    assert!(rule_idx < title_idx && title_idx < row_idx, "band ═ title ═ rows order");
}
...
fn open_lots_title_row_aligns_with_its_columns() {
    ...
    for t in ["LOT", "SYMBOL", "NAME", "DATE", "SOURCE", "TERM", "REM", "BASIS", "$/SH", "PLATFORM", "CODE"] {
        assert!(title.text().contains(t), "the {t} title renders: {}", title.text());
    }
    assert_eq!(title.span_role("PLATFORM"), Some(tui::theme::Role::FgFaint));
    let a = lines.iter().find(|l| l.text().contains("L-A ")).unwrap();
    let z = lines.iter().find(|l| l.text().contains("L-PRIVHLD")).unwrap();
    assert_eq!(rule_positions(title), rule_positions(a), "title aligns:\n{}\n{}", title.text(), a.text());
    assert_eq!(rule_positions(title), rule_positions(z), "title aligns:\n{}\n{}", title.text(), z.text());
}
...
fn positions_render_the_company_name_with_ticker_fallback_and_clamped_ellipsis() {
    ...
    assert!(row("AMZN").text().contains("Amazon.com"), "{}", row("AMZN").text());
    // An over-clamp name is truncated with … — and never shears the grid.
    let qubt = row("QUBT");
    assert!(qubt.text().contains('…'), "the long name ellipsizes: {}", qubt.text());
    assert!(!qubt.text().contains("Incorporated"), "clamped: {}", qubt.text());
    assert_eq!(rule_positions(row("AMZN")), rule_positions(qubt), "the grid holds");
    ...
    assert!(pltr_text.matches("PLTR").count() >= 2, "ticker fallback: {pltr_text}");
}
...
fn open_lots_render_the_company_name_with_ticker_fallback() {
    ...
    let view = ViewBuilder::new(snap).display_name("AMZN", "Amazon.com").build();
    let lines = screen_lines(&view, &NavState::new(Screen::OpenLots));
    let a = lines.iter().find(|l| l.text().contains("L-A ")).unwrap();
    assert!(a.text().contains("Amazon.com"), "the lot row names the company: {}", a.text());
    let p = lines.iter().find(|l| l.text().contains("L-P ")).unwrap();
    assert!(p.text().matches("PLTR").count() >= 2, "ticker fallback: {}", p.text());
    assert_eq!(rule_positions(a), rule_positions(p), "the grid holds with the name column");
}
