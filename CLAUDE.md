...
A personal stock-portfolio tracking application for a single owner-operator. It tracks
equity and RSU positions held across multiple trading platforms, computes realized and
unrealized profit and tax on a tax-lot basis, manages per-sale tax accruals through their
lifecycle (accrued → allocated → moved → paid), and produces portfolio and quarterly-tax
reports. Activity is entered through a local terminal UI (TUI); a clean, filterable,
read-only view is published to Google Sheets.

It replaces a hand-wired Google Sheet (tabs: Positions, Stock Actions, Stock Sales) and the
owner's `portfolio-summary.sh` daily-summary script that scraped it.

## Tech stack

- **Rust (stable)** for all components.
- **Verus and Kani** applied selectively to the pure accounting/tax core, in the same way
  as a prior private verified-Rust project: a `verus!{}` module that erases under stable `cargo build` and is proven
  under the pinned Verus toolchain, with `#[cfg(kani)]` bounded harnesses alongside.
- **Google Sheets** is the canonical store (via the `sheets` CLI / Sheets API): append-only
  event-log tabs written through by the TUI, plus engine-projected view tabs with a
  `GOOGLEFINANCE` column as the live-price oracle. Serde and all I/O live *outside* the
  `verus!{}` boundary as the single trust seam (Sheets row ↔ event).
- A **rebuildable local cache** mirrors the event log for fast/offline reads; it carries no
  truth and can be rebuilt from the workbook at any time.
- **ratatui + crossterm** (immediate-mode) for the TUI, with `tui-textarea` for input fields and
  a hand-rolled Model–Update–View loop. Outside the verified boundary.

## Doc conventions

- **Diagrams are ASCII, not Mermaid** — in the HLD, LLDs, and all design docs.
