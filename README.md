...
- **The money math is formally verified.** The pure accounting/tax kernels live in
  ...
  [`docs/notes/verus-and-kani.md`](docs/notes/verus-and-kani.md) is the deep dive — what the
  two tools are, a tour of the actual proofs, and an honest account of the tradeoffs.
...
- The **HLD** (high-level design, [`docs/high-level-design.md`](docs/high-level-design.md))
  states the problem, the approach, goals/non-goals, and ordered tenets.
- Each component gets an **LLD** (low-level design) under [`docs/intent/`](docs/intent/) —
  eleven segments, from the verified
  [`ledger-core`](docs/intent/ledger-core/ledger-core-design.md) kernel to the one-time
  legacy [`import`er](docs/intent/import/import-design.md).
- Each LLD's behavior is pinned as **EARS** specs — one-line structured requirements,
  ...
  [`docs/intent/store/store-specs.md`](docs/intent/store/store-specs.md):
...
- A navigation overlay in [`docs/arrows/`](docs/arrows/README.md)
  ([`index.yaml`](docs/arrows/index.yaml) + one orientation page per segment) tracks each
  segment's audit status and dependency graph, so a session can orient in seconds without
...
**A guided tour, six files in order:**

1. [`docs/high-level-design.md`](docs/high-level-design.md) — the problem, tenets, and
   system shape.
2. [`docs/intent/store/store-design.md`](docs/intent/store/store-design.md) — one LLD: the
   Sheets↔event trust seam, the read-back-verify write path, the two-tier cache fingerprint.
3. [`docs/intent/store/store-specs.md`](docs/intent/store/store-specs.md) — that design
   pinned as 30 EARS lines.
4. [`crates/store/tests/write.rs`](crates/store/tests/write.rs) — the `@spec`-annotated
   tests that enforce them.
5. [`scripts/ci.sh`](scripts/ci.sh) — the gate that binds it all: build, test, Verus, Kani,
   spec coverage, live e2e.
6. [`docs/notes/verus-and-kani.md`](docs/notes/verus-and-kani.md) — the deep dive on the
   verified slice that gate runs: what Verus and Kani are, a tour of the actual proofs, and an
   honest account of the tradeoffs.

To see how a session navigates all eleven segments at once — audit status, dependency
graph, and the path from any spec to its code — start at the overlay's own guide,
[`docs/arrows/README.md`](docs/arrows/README.md).
...
is in [`docs/notes/build-process.md`](docs/notes/build-process.md). Two bugs the discipline
caught are worth retelling:
...
[`docs/runbooks/refresh-tax-brackets.md`](docs/runbooks/refresh-tax-brackets.md) is the
maintenance runbook (the TUI reminds you when
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) is a thin host around
[`scripts/ci.sh`](scripts/ci.sh) — the gate is identical
...
the arrow — find the owning LLD under [`docs/intent/`](docs/intent/), adjust the design and
its EARS, then tests, then code. The [`CLAUDE.md`](CLAUDE.md) and the
[`docs/arrows/`](docs/arrows/README.md) overlay will orient you; the coverage gate will keep
you honest.
