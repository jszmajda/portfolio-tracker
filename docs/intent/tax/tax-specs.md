...
- [x] **TAX-ACCRUAL-009**: While a combined migration accrual (TAX-ACCRUAL-007) exists for a `(jurisdiction, tax_year)`, the system shall supersede that year's per-`RealizedGain` accruals — excluding them from outstanding — so the year reconciles to the seeded legacy actual.
...
- [x] **TAX-REPORT-005**: When aggregating per-`(jurisdiction, tax_year)` figures, the annual report (TAX-REPORT-003) and the quarterly sales report (TAX-REPORT-001/002) shall own applying the de-minimis (TAX-ACCRUAL-005) and migration-supersession (TAX-ACCRUAL-009) exclusions — excluding any accrual marked auto-settled (`|amount|` below de-minimis) or superseded from the accrued/moved/paid/outstanding/shortfall and per-period accrual sums — so a migrated year reconciles to the seeded legacy actual with no double-count and rounding-epsilon accruals add nothing.
